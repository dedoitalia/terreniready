...
export default function TerreniDashboard() {
  ...
  // Cancel handler: chiude l'EventSource lato client e resetta lo stato
  // di loading. Il job sul server continua fino al completamento (non
  // abbiamo ancora abort signal end-to-end per Overpass), ma l'utente
  // riottiene subito la UI. Il prossimo run della stessa scansione
  // riciclera` i risultati cacheati quando il server li emette.
  const cancelScan = useCallback(() => {
    ...
    setScanJob((current) => ({
      status: "failed",
      error: "Scansione annullata dall'utente. Il motore continua in background fino al completamento naturale.",
      logs: current?.logs ?? [],
    }));
    setError(
      "Scansione annullata. I risultati parziali ricevuti prima dell'annullamento restano in mappa.",
    );
  }, [clearStreamReconnectWatchdog]);
              ...
              {loading ? (
                <div className="mt-5 rounded-[24px] border border-white/10 bg-white/6 p-4 text-sm leading-6 text-[#edf2e7]">
                  ...
                  <button
                    type="button"
                    onClick={cancelScan}
                    className="mt-3 inline-flex w-full items-center justify-center rounded-full border border-[rgba(235,140,123,0.32)] bg-[rgba(176,72,56,0.18)] px-4 py-2 text-xs font-medium uppercase tracking-[0.18em] text-[#ffe3db] transition hover:bg-[rgba(176,72,56,0.28)]"
                  >
                    Annulla scansione
                  </button>
                </div>
